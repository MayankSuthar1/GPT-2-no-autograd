"""
Train gpt_ffn_bp_v3_cupy.GPT2 on Tiny Shakespeare without PyTorch autograd using CuPy.

Default usage from the repo root:
  python no_autograd/train_gpt_ffn_bp_v3_cupy.py
...
from gpt_ffn_bp_v3_cupy import CrossEntropyLoss, GPT2
...
out_dir = "out-gpt-ffn-bp-v3-cupy-openwebtext"
seed = 1337
eval_interval = 250      # Increased: OpenWebText evaluations take longer
log_interval = 10
eval_iters = 20         # Increased for a more stable validation loss reading
eval_only = False
...
batch_size = 16                  # Massive batch size to keep the A100's tensor cores busy
block_size = 1024                 # Full standard GPT-2 context window

# model
n_layer = 10             # Increased depth
n_head = 4               # 128 / 4 = 32 dimensions per head
n_embd = 128             # Carefully calculated to fit the 15M budget
vocab_size = 50304       # Standard GPT-2 vocab

# optimizer
learning_rate = 6e-4     # Slightly lower max LR for a larger model
max_iters = 60000        # Vastly increased. OpenWebText needs many more steps.
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
grad_clip = 1.0
...
lr_decay_iters = 60000   # Should match max_iters
min_lr = 6e-5
...
wandb_run_name = "gpt-ffn-bp-v3-cupy-15M"
...
class ParameterRef:
    ...
    def zero_grad(self):
        setattr(self.owner, self.grad_name, cp.zeros_like(self.data))
...
class AdamW:
    def __init__(self, parameters, lr, betas, weight_decay, eps=1e-8):
        ...
        self.eps = eps
        self.t = 0
        self.m = [cp.zeros_like(p.data) for p in parameters]
        self.v = [cp.zeros_like(p.data) for p in parameters]
...
def clip_grad_norm(parameters, max_norm):
    ...
    total_sq = 0.0
    for grad in grads:
        grad64 = grad.astype(cp.float64, copy=False)
        total_sq += float(cp.sum(grad64 * grad64))
...
def ensure_dataset(dataset, prepare_data):
    # 1. Look for the dataset in our persistent volume first
    data_dir = Path(os.environ.get("DATA_DIR", ROOT_DIR / "data" / dataset))
    data_dir.mkdir(parents=True, exist_ok=True)
    
    required = [data_dir / "train.bin", data_dir / "val.bin"]
    if all(path.exists() for path in required):
        print(f"Found existing dataset in {data_dir}. Skipping preparation.")
        return data_dir

    # 2. If missing, find the prepare.py script in your local files and run it
    prepare_script = ROOT_DIR / "data" / dataset / "prepare.py"
...
def get_batch(data, batch_size, block_size, rng):
    ...
    x_np = np.stack([data[i : i + block_size].astype(np.int64) for i in ix])
    y_np = np.stack([data[i + 1 : i + 1 + block_size].astype(np.int64) for i in ix])
    x = cp.array(x_np)
    y = cp.array(y_np)
    return x, y
...
def estimate_loss(model, train_data, val_data, batch_size, block_size, eval_iters, rng):
    ...
    for split, data in (("train", train_data), ("val", val_data)):
        ...
        for _ in range(eval_iters):
            ...
            losses.append(float(loss_fn.forward(logits, y)))
...
def save_checkpoint(path, parameters, iter_num, best_val_loss):
    arrays = {p.key: cp.asnumpy(p.data) for p in parameters}
...
def sample_text(model, meta, prompt, block_size, max_new_tokens, temperature, rng):
    ...
    for _ in range(max_new_tokens):
        x = cp.array([ids[-block_size:]], dtype=cp.int64)
        logits = model.forward(x)[0, -1, :] / max(temperature, 1e-6)
        logits = logits - cp.max(logits)
        probs = cp.exp(logits)
        probs = probs / cp.sum(probs)
        probs_np = cp.asnumpy(probs)
        ids.append(int(rng.choice(len(probs_np), p=probs_np)))
...
def main():
    ...
    for iter_num in range(max_iters + 1):
        ...
        if iter_num % eval_interval == 0:
            ...
            if metrics["val"]["loss"] < best_val_loss or always_save_checkpoint:
                ...
                save_checkpoint(output_dir / "ckpt_best_v3_cupy.npz", parameters, iter_num, best_val_loss)
                print(f"saved best checkpoint to {display_path(output_dir / 'ckpt_best_v3_cupy.npz')}")

        if iter_num == 0 and eval_only:
            break

        accumulated_grads = [cp.zeros_like(p.data) for p in parameters]
        loss_total = 0.0
        for _ in range(gradient_accumulation_steps):
            ...
            loss_total += float(loss) / gradient_accumulation_steps
    ...
    save_checkpoint(output_dir / "ckpt_last_v3_cupy.npz", parameters, max_iters, best_val_loss)
    print(f"saved last checkpoint to {display_path(output_dir / 'ckpt_last_v3_cupy.npz')}")
