...
class Embedding:
    def __init__(self, vocab_size, embed_dim):
        self.weight = cp.random.randn(vocab_size, embed_dim) * 0.02
    ...
    def backward(self, grad_output):
        ...
        cupyx.scatter_add(self.grad_weight, self.inputs, grad_output)
        # We do not pass gradients back past embeddings since the inputs are integers
        return None
    
class Linear:
    def __init__(self, input_dim, output_dim):
        self.weight = cp.random.randn(output_dim, input_dim) * 0.02 # [output_features, input_features]
        self.bias = cp.zeros(output_dim)

    def forward(self, x):
        self.x = x  # Store input for backward pass
        return cp.dot(x, self.weight.T) + self.bias # Perform matrix multiplication
    ...
    def backward(self, grad_output):
        ...
        grad_w = cp.dot(grad_out_reshaped.T, x_reshaped) 
        grad_b = cp.sum(grad_out_reshaped, axis=0)
        ...
        grad_input = cp.dot(grad_output, self.weight)
...
class GELU:
    def forward(self, x):
        self.x = x
        self.inner = cp.sqrt(2 / cp.pi) * (x + 0.044715 * cp.power(x, 3))
        self.tanh = cp.tanh(self.inner)
    ...
    def backward(self, grad_output):
        ...
        pdf = 0.5 * cp.sqrt(2 / cp.pi) * sech2 * (1.0 + 3.0 * 0.044715 * cp.power(x, 2))
...
class Softmax:
    ...
    def backward(self, grad_output):
        ...
        sum_grad_y = cp.sum(grad_output * self.output, axis=-1, keepdims=True)
...
class LayerNorm:
    def __init__(self, dim, eps=1e-5):
        self.gamma = cp.ones(dim)
        self.beta = cp.zeros(dim)
        self.eps = eps
    ...
    def backward(self, grad_output):
        ...
        self.grad_gamma = cp.sum(grad_output * self.x_norm, axis=axes)
        self.grad_beta = cp.sum(grad_output, axis=axes)
        
        # Gradient for input passed down the network
        dx_norm = grad_output * self.gamma
        dx = (1.0 / N) / self.std * (
            N * dx_norm -
            cp.sum(dx_norm, axis=-1, keepdims=True) -
            self.x_norm * cp.sum(dx_norm * self.x_norm, axis=-1, keepdims=True)
        )
        return dx
...
class AttentionHead:
    def __init__(self, embd_size, head_size, n_positions=1024):
        ...
        self.tril = cp.tril(cp.ones((n_positions, n_positions)))
        self.scale = cp.sqrt(head_size)
    ...
    def forward(self, x):
        ...
        self.attn_scores = cp.matmul(self.q, self.k_T) / self.scale
        
        # 3. Masking
        mask = self.tril[:self.seq_len, :self.seq_len]
        self.attn_scores = cp.where(mask == 0, -cp.inf, self.attn_scores)
        
        # 4. Softmax
        self.wei = self.softmax_layer.forward(self.attn_scores)
        
        # 5. Multiply weights by Values
        out = cp.matmul(self.wei, self.v)
        return out
    ...
    def backward(self, grad_output):
        ...
        grad_v = cp.matmul(self.wei.transpose(0, 2, 1), grad_output)
        # d_wei = grad_output @ v^T
        grad_wei = cp.matmul(grad_output, self.v.transpose(0, 2, 1))
        ...
        mask = self.tril[:self.seq_len, :self.seq_len]
        grad_attn_scores = cp.where(mask == 0, 0, grad_attn_scores)
        ...
        grad_q = cp.matmul(grad_attn_scores, self.k)
        # d_k = grad_attn_scores^T @ q
        grad_k = cp.matmul(grad_attn_scores.transpose(0, 2, 1), self.q)
...
class MultiHeadAttention:
    ...
    def forward(self, x):
        ...
        concat = cp.concatenate(head_outputs, axis=2)
    ...
    def backward(self, grad_output):
        ...
        grad_heads = cp.split(grad_concat, self.num_heads, axis=2)
...
class GPT2:
    ...
    def forward(self, inputs):
        ...
        positions = cp.arange(self.seq_len)
...
class CrossEntropyLoss:
    ...
    def forward(self, logits, targets):
        ...
        shifted_logits = logits_flat - cp.max(logits_flat, axis=-1, keepdims=True)
        ...
        correct_probs = self.probs[cp.arange(N), self.targets]
        loss = -cp.sum(cp.log(correct_probs + 1e-10)) / N
        return loss
    ...
    def backward(self):
        ...
        grad_logits[cp.arange(N), self.targets] -= 1
...
class AdamOptimizer:
    def __init__(self, parameters, lr=1e-3, beta1=0.9, beta2=0.999, eps=1e-8):
        ...
        self.m = {id(p): cp.zeros_like(p.data) for p in parameters}
        self.v = {id(p): cp.zeros_like(p.data) for p in parameters}
...
if __name__ == "__main__":
    ...
    inputs = cp.array([token_ids])  # Shape (1, sequence_length)
    ...
    text_output = tokenizer.decode(cp.asnumpy(cp.argmax(output, axis=-1)).flatten().tolist())
    print(text_output)  # Print the decoded output text (may not be meaningful due to random weights)
