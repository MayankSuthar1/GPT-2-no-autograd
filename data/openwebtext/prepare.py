...
if __name__ == '__main__':
    ...
    def process(examples):
        # examples['text'] is now a list of strings instead of a single string
        all_ids = []
        all_lens = []
    ...
    # tokenize the dataset using batched=True
    tokenized = split_dataset.map(
        process,
        batched=True,          # <--- VITAL: Processes rows in groups
        batch_size=1000,       # <--- Group size sent to each process
        remove_columns=['text'],
